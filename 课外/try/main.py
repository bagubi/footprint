...
# 检查是否存在联系人文件，如果不存在，则创建一个空的联系人文件
...
class AddressBook:
    ...
    方法:
    - load_contacts: 加载联系人信息 from 文件。
    - save_contacts: 将联系人信息保存 to 文件。
    - add_contact: 添加新的联系人。
    - display_contacts: 显示所有联系人。
    - search_contact: 根据关键词搜索联系人。
    - edit_contact: 编辑现有联系人。
    ...
    def __init__(self, filename="main.json"):
        """
        初始化通讯录类实例，设定文件名默认为'main.json'。
        
        参数:
        - filename: 存储联系人数据的文件名，默认为'main.json'。
        """
    ...
    def load_contacts(self):
        """
        从文件加载联系人信息，并创建Contact对象列表。
        
        尝试打开文件并解析JSON，如果文件不存在或JSON格式不正确，则打印错误消息。
        """
        try:
    ...
    def add_contact(self):
        ...
        循环直到获取有效输入，然后创建新的Contact对象并添加到通讯录中。
        """
        while True:
            name = input("请输入姓名：")
            if not name:
...
def main():
    ...
    创建AddressBook实例，并提供一个交互式菜单供用户添加、显示、搜索、编辑和删除联系人，以及保存并退出应用程序。
    """
